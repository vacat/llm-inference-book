...
**Q8_0 块结构（关键：每块 34 字节）**

Q8_0 把向量按 32 个元素一组切块，每块存两样东西：

```
Q8_0 块 (34 字节):
  [scale (F16, 2 字节)] [q0 q1 q2 ... q31 (int8, 32 字节)]
  └── 1 个缩放因子 ─────┘└── 32 个量化值 ─────────────────┘
                       总计 2 + 32 = 34 字节
```

- **F16 缩放因子 (2 字节)**：这一块所有量化值共享一个最大值，先把 32 个 float 元素按 max(|x|) 归一化到 [-127, 127]，再 int8 量化；反量化时乘回这个 scale。
- **32 个 int8 (32 字节)**：实际量化值。
- **34 字节是固定常量**：不管 n 是不是 32 的倍数，每块永远 34 字节。这个 34 就是代码里 `* 34` 偏移的来源。

读第 `token` 行 = 把指针移动 `token × blocks × 34` 字节；每块内部 `+ 0` 取 scale、`+ 2` 取量化值。下面的代码完整演示了这套规则：
